...
#define DBG_NEG(d, i)                                                          \
  if ((d) <= R0) {                                                             \
    CHRw(stdout, "Negative Jacobian for element ");                            \
    INTw(stdout, i, 5);                                                        \
    CHRwcr(stdout);                                                            \
  }
#else
...
static INLINE void
calFragVol(INT nelem, INT *i1elfrg, INT *NElemSet, DBL *VolElemSet,
           DBL *d1elvol, INT **i2eljp, INT *i1elpr, INT *i1pejp, INT **i2ejjp,
           INT *i1elchprm, INT *i1nochprm, INT **i2elto, DBL *d1nccx,
           DBL *d1nccy, DBL *d1nccz, DBL *d1nvcx, DBL *d1nvcy, DBL *d1nvcz,
           INT nelno, INT ichprm, INT iremoval, INT indexv, DBL dcrmvx,
           DBL dcrmvy, DBL dcrmvz, DBL **d2nfrm, INT *i1elfrm, INT *i1frgrm,
           DBL *d1ndisp, DBL dcfrmca, INT *i1nopr, INT *i1bnvx, INT *i1bnvy,
           INT *i1bnvz, INT *i1rmgrav, DBL *d1mart) {
 ...
 */
static INLINE void jacobi(DBL d2xj[NDIME][NDIME], DBL **d2ncc, DBL **d2dsh,
                          INT *i1elto) {
  INT i, j, k, ip;
  DBL *d1dsh;
  DBL *d1ncc, *d1xj;
 ...
 */
static INLINE void jacobi2(DBL d2xj[NDIME][NDIME], DBL **d2ncc,
                           DBL d2xjci[NDIME][NDIME], DBL **d2nci, DBL **d2dsh,
                           INT *i1elto) {
  INT i, j, k, ip;
  DBL *d1dsh;
  DBL *d1ncc, *d1xj;
 ...
 */
static INLINE void grad(DBL d2nd[NDIME][NNODE], DBL d2xjinv[NDIME][NDIME],
                        DBL **d2dsh) {
  INT i;

  for (i = 0; i < NNODE; i++) {
    d2nd[0][i] = (d2xjinv[0][0] * d2dsh[0][i]) + (d2xjinv[1][0] * d2dsh[1][i]) +
                 (d2xjinv[2][0] * d2dsh[2][i]);
    d2nd[1][i] = (d2xjinv[0][1] * d2dsh[0][i]) + (d2xjinv[1][1] * d2dsh[1][i]) +
                 (d2xjinv[2][1] * d2dsh[2][i]);
    d2nd[2][i] = (d2xjinv[0][2] * d2dsh[0][i]) + (d2xjinv[1][2] * d2dsh[1][i]) +
                 (d2xjinv[2][2] * d2dsh[2][i]);
  }
}
 ...
 */
static INLINE void ctgrad(DBL d2nd[NDIME][NNODE], DBL d2finv[NDIME][NDIME],
                          DBL **d2nci, INT *i1elto) {
  INT i, j, k, ip;
  DBL *d1nd;
  DBL *d1finv, *d1nci;
...
static INLINE void ctvgrad(DBL d2nd[NDIME][NNODE], DBL d2finv[NDIME][NDIME],
                           DBL **d2nci, DBL d2vgrad[NDIME][NDIME], DBL **d2nvc,
                           INT *i1elto) {
  INT i, j, k, ip;
  DBL *d1nd;
  DBL *d1finv, *d1nci;
...
static INLINE void cauchy(DBL T[NDIME][NDIME], DBL L[NDIME][NDIME],
                          DBL d2ftens[NDIME][NDIME],
                          //         DBL ***d3tcs,
                          DBL dpemu, DBL dpela, DBL detf, DBL detf0, DBL dpeks,
                          INT ielem) {
  INT i, j;
  ...
  for (i = 0; i < NDIME; i++) {
    ...
    for (j = 0; j < NDIME; j++) {
      ...
      B = ((d2ftens[i][0] * d2ftens[j][0]) + (d2ftens[i][1] * d2ftens[j][1]) +
           (d2ftens[i][2] * d2ftens[j][2])) *
          scale1;
 ...
 */
static INLINE void stract(DBL d2st[NDIME][NNODE], DBL T[NDIME][NDIME],
                          DBL d2nd[NDIME][NNODE], DBL detjb) {
  INT i;

  for (i = 0; i < NNODE; i++) {
    d2st[0][i] = ((d2nd[0][i] * T[0][0]) + (d2nd[1][i] * T[0][1]) +
                  (d2nd[2][i] * T[0][2])) *
                 detjb;
    d2st[1][i] = ((d2nd[0][i] * T[1][0]) + (d2nd[1][i] * T[1][1]) +
                  (d2nd[2][i] * T[1][2])) *
                 detjb;
    d2st[2][i] = ((d2nd[0][i] * T[2][0]) + (d2nd[1][i] * T[2][1]) +
                  (d2nd[2][i] * T[2][2])) *
                 detjb;
  }
}
 ...
 */
static INLINE void nforce(DBL *d1nftx, DBL *d1nfty, DBL *d1nftz,
                          DBL d3st[NGRSH][NDIME][NNODE], INT *i1elto) {
  INT i, ip;
  ...
  for (i = 0; i < NNODE; i++) {
    ...
    d1nftx[ip] -= WEIGHT * (d3st[1][0][i] + d3st[2][0][i] + d3st[3][0][i] +
                            d3st[4][0][i]);
    d1nfty[ip] -= WEIGHT * (d3st[1][1][i] + d3st[2][1][i] + d3st[3][1][i] +
                            d3st[4][1][i]);
    d1nftz[ip] -= WEIGHT * (d3st[1][2][i] + d3st[2][2][i] + d3st[3][2][i] +
                            d3st[4][2][i]);
 ...
 */
static void Yfd3TET10ELS(INT nelem, DBL *dpeks, DBL *dpela, DBL *dpemu,
                         DBL **d2ncc, DBL **d2nci, DBL **d2nft, DBL **d2nvc,
                         INT *i1elpr, INT **i2elto, DBL ***d3dsh, INT nthreads,
                         INT nblock, INT nnopo, INT nmode) {

#pragma omp parallel default(none)                                             \
    shared(nelem, dpeks, dpela, dpemu, d2ncc, d2nci, d2nft, d2nvc, i1elpr,     \
               i2elto, d3dsh, nnopo, nthreads, nblock, nmode)
  {
...
#pragma omp for schedule(guided) // nowait
    for (ielem = 0; ielem < nelem; ielem++) {
      ...
      for (ig = 1; ig < NGRSH; ig++) {
        ...
        /* calculate left Cauchy-Green strain tensor B, deformation rate D,
         * Cauchy stress T, net stress tcs */
        cauchy(T, L, d2ftens, dpemu[iprop], dpela[iprop], detf, detf0,
               dpeks[iprop], ielem);
...
static INLINE DBL FdftensTet4(DBL F[3][3], DBL L[3][3], DBL *d1nccx,
                              DBL *d1nccy, DBL *d1nccz, DBL *d1ncix,
                              DBL *d1nciy, DBL *d1nciz, DBL *d1nvcx,
                              DBL *d1nvcy, DBL *d1nvcz, INT *i1elto,
                              DBL *d1elvol) {
  INT j, k;
  int i, ii;
...
static INLINE void CauchyTet4_pore_pressure(DBL F[3][3], DBL L[3][3], DBL dpemu,
                                            DBL dpeks, DBL dpela, DBL detf,
                                            DBL T[3][3], DBL D[3][3]) {
  ...
  dups_u = dups / 0.8; /* using 20% or lower approaximate at the moment -
                          Devonian Shale; Hydrostone -   */
  dups_u = 0.3839;
  ...
  Tm = -(detf - 1.0) * 2.0 * SK * dpemu * (1.0 + dups_u) /
       (3.0 * (1.0 - 2.0 * dups_u));
  Tm = R0;
  for (i = 0; i < 3; i++) {
...
static INLINE void CauchyTet4(DBL F[3][3], DBL L[3][3], DBL dpemu, DBL dpeks,
                              DBL dpela, DBL detf, DBL T[3][3], DBL D[3][3],
                              DBL *dem0n, DBL dem0, DBL demf, DBL dpedm,
                              DBL *deldam) {
  DBL B[3][3]; /* left Cauchy-Green strain tensor */
  int i, j, k;
  DBL df;
...
static INLINE void nforceTet4(DBL T[3][3], DBL *d1nccx, DBL *d1nccy,
                              DBL *d1nccz, DBL *d1nfcx, DBL *d1nfcy,
                              DBL *d1nfcz, INT *i1elto, DBL *d1elfsx,
                              DBL *d1elfsy, DBL *d1elfsz) {
  DBL nx, ny, nz;
  int i;
  INT ii, jj, kk, ll;
  DBL tx, ty, tz;
  ...
  for (i = 0; i < 4; i++) /* Nodal Forces */
  {
    ...
    kk = i1elto[p2[i]];
    ll = i1elto[p3[i]];
    nx = ((d1nccy[kk] - d1nccy[jj]) * (d1nccz[ll] - d1nccz[jj]) -
          (d1nccy[ll] - d1nccy[jj]) * (d1nccz[kk] - d1nccz[jj])) /
         R6;
    ny = ((d1nccz[kk] - d1nccz[jj]) * (d1nccx[ll] - d1nccx[jj]) -
          (d1nccx[kk] - d1nccx[jj]) * (d1nccz[ll] - d1nccz[jj])) /
         R6;
    nz = ((d1nccx[kk] - d1nccx[jj]) * (d1nccy[ll] - d1nccy[jj]) -
          (d1nccy[kk] - d1nccy[jj]) * (d1nccx[ll] - d1nccx[jj])) /
         R6;
...
static void Yfd3TET4ELS(/* small strain elastic 4-noded tetrahedra  */
                        INT nelem, INT *i1elpr, DBL *d1peks, DBL *d1pela,
                        DBL *d1pemu, DBL *d1nccx, DBL *d1nccy, DBL *d1nccz,
                        DBL *d1ncix, DBL *d1nciy, DBL *d1nciz, DBL **d2nft,
                        DBL *d1nvcx, DBL *d1nvcy, DBL *d1nvcz, INT **i2elto,
                        DBL *d1elvol, INT *i1remov, DBL *d1m0n, DBL *d1m0,
                        DBL *d1mf, DBL *d1pedm, DBL *d1eldam, DBL ***d3tst
...
#pragma omp parallel default(none)                                             \
    shared(nelem, i1elpr, d1peks, d1pela, d1pemu, d1nccx, d1nccy, d1nccz,      \
               d1ncix, d1nciy, d1nciz, d2nft, d1nvcx, d1nvcy, d1nvcz, i2elto,  \
               d1elvol, i1remov, d1m0n, d1m0, d1mf, d1eldam, d1pedm, d3tst,    \
...
#pragma omp for schedule(guided) nowait
    ...
    for (i = 0; i < nelem; i++) {
      if (i1remov[i] != 1) {
        ...
        detf = FdftensTet4(F, L, d1nccx, d1nccy, d1nccz, d1ncix, d1nciy, d1nciz,
                           d1nvcx, d1nvcy, d1nvcz, i1elto, &d1elvol[i]);
        CauchyTet4(F, L, d1pemu[iprop], d1peks[iprop], d1pela[iprop], detf, T,
                   D, &d1m0n[i], d1m0[iprop], d1mf[iprop], d1pedm[iprop],
                   &d1eldam[i]);
        ...
        nforceTet4(T, d1nccx, d1nccy, d1nccz, d2nft[0], d2nft[1], d2nft[2],
                   i1elto, d2elfsx[i], d2elfsy[i], d2elfsz[i]);
      }
    }
  }
...
static INLINE void S_N_direction(DBL o[3], DBL s[3], DBL normal[3], DBL sx[3],
                                 DBL sy[3], DBL sz[3], DBL *area, DBL *el,
                                 INT in[6], DBL *d1nccx, DBL *d1nccy,
                                 DBL *d1nccz, INT ieljo) {

  DBL prjx[3], prjy[3],
      prjz[3]; /*projection of first 3 joint element nodes on middle plane*/
  DBL mpx[6], mpy[6], mpz[6]; /*middle points of the 6 edges: 0-01, 1-12, 2-20,
                                 3-54, 4-43, 5-35*/
  DBL midx[3], midy[3],
      midz[3]; /*coordinates of middle points; 0-05, 1-14, 2-23*/
  DBL midy10, midz20, midz10, midy20, midx20, midx10, midx12, midy12, midz12;
  DBL tmp, tmp1, tmp2, tmp3, mpx03, mpy03, mpz03, mpx14, mpy14, mpz14, mpx25,
      mpy25, mpz25;
  DBL small, h;
  small = EPSILON;
  ...
  /* in numerical integration, the area used in the calculation of node force is
   * 1/6 of the triangle area */
  *area = h / 12.0;
  ...
  *el = (SQRT(midx10 * midx10 + midy10 * midy10 + midz10 * midz10) +
         SQRT(midx12 * midx12 + midy12 * midy12 + midz12 * midz12) +
         SQRT(midx20 * midx20 + midy20 * midy20 + midz20 * midz20)) /
        R3;
...
static INLINE void
Sigma_tau(DBL o[3], DBL s[3], DBL sx[3], DBL sy[3], DBL sz[3], DBL normal[3],
          DBL area, DBL el, DBL *d1nftx, DBL *d1nfty, DBL *d1nftz, DBL dpeft,
          DBL dpegfn, DBL dpegfs, DBL dpepe, DBL dpcoh, DBL dpicf, INT **i2eljp,
          INT **i2ejjp, INT *i1elpr, INT *i1elty, INT ielem, INT *i1elbe,
          int tid, INT *index, INT in[6], INT iprop, INT *nelem_t,
          INT **i2elto_t, INT *i1bd, DBL *d1nbp, DBL dbkp, DBL *d1ejfcx,
          DBL *d1ejfcy, DBL *d1ejfcz, INT *i1eljt, INT *i1elncstep, INT ncstep,
          DBL *d1ejfjx, DBL *d1ejfjy, DBL *d1ejfjz) {
  DBL odis, sdis, sabs, op, sp, ot, st, z, sigma, tau, sigma_tmp, tmp1, tmp2,
      tmp, tmp3;
  DBL dpefa = 0.63;
  DBL dpefb = 1.8;
  DBL dpefc = 6.0;
  ...
  for (integ = 0; integ < 3; integ++) {
    ...
    if (z >= R1) {
      ...
      if ((nfail > 1) && (i1elpr[ielem] >= 0)) {
        ...
        //-PY changed it for 3D_fracture_coupling_with_multiphase -ao commented
        //this again

        for (i = 0; i < 4; i++) {
          if (i2eljp[i2ejjp[ielem][0]][i] == ielem) {
            i2eljp[i2ejjp[ielem][0]][i] =
                -(i2eljp[i2ejjp[ielem][0]][i] + YIPROPMAX);
          }
          if (i2eljp[i2ejjp[ielem][1]][i] == ielem) {
            i2eljp[i2ejjp[ielem][1]][i] =
                -(i2eljp[i2ejjp[ielem][1]][i] + YIPROPMAX);
          }
        }
#pragma omp critical
        {
...
#pragma omp atomic
    ...
    if (iz == 0)
      ...
      z = (R1 - ((dpefa + dpefb - R1) / (dpefa + dpefb)) *
                    exp(z * (dpefa + dpefc * dpefb) /
                        ((dpefa + dpefb) * (R1 - dpefa - dpefb)))) *
    ...
    if ((sigma > R0) && (sabs > sp)) {
      ...
      tau = (R2 * tmp - tmp * tmp) *
            (z * dpefs - dpefm * sigma); //-ao getting NaNs sometimes
    }
...
Yfd3TET4JOINT(/* joint element for 4-node tetrahedra */
              INT nelem, INT *i1ptyp, DBL *dpefti, DBL *dpegfni, DBL *dpegfsi,
              DBL *dpepei, DBL *d1nccx, DBL *d1nccy, DBL *d1nccz, DBL *d1nftx,
              DBL *d1nfty, DBL *d1nftz, DBL *d1nvcx, DBL *d1nvcy, DBL *d1nvcz,
              INT *i1elpr, INT **i2elto, INT *i1eljo, INT **i2eljp,
              INT **i2ejjp, DBL *d0iedi, INT **i2elfr, INT *i1elbe,
              INT *i1joint, INT njoint, INT *ifrac, DBL *dpcohi, DBL *dpicf,
              INT *i1elty, INT *i1elst, INT *nelem_t, INT **i2elto_t, INT *i1bd,
              DBL *d1nbp, DBL dbkp, DBL *d1ejfcx, DBL *d1ejfcy, DBL *d1ejfcz,
              INT *i1defect, int nthreads, INT *i1eljt, DBL *d1ejmr,
...
#pragma omp parallel default(none)                                             \
    shared(nelem, dpefti, dpegfni, dpegfsi, dpepei, d1nccx, d1nccy, d1nccz,    \
               d1nftx, d1nfty, d1nftz, d1nvcx, d1nvcy, d1nvcz, i1elpr, i2elto, \
               i1eljo, i2eljp, i2ejjp, d0iedi, i2elfr, i1elbe, i1ptyp, i1elst, \
               d1ejfcx, d1ejfcy, d1ejfcz, i1elncstep, ncstep, d1ejmr, dpcohi,  \
               dpicf, i1elty, index, i1joint, njoint, nelem_t, i2elto_t, i1bd, \
               d1nbp, dbkp, i1defect, i1eljt, d3tst, d2jst, stdout, d2ejfjx,   \
               d2ejfjy, d2ejfjz, i1elpr_old)
  {
...
#pragma omp for schedule(guided) nowait
    ...
    for (ielem = 0; ielem < nelem; ielem++) {
      ...
      if ((iprop >= 0) && (i2elto[ielem][0] != i2elto[ielem][5])) {
        S_N_direction(o, s, normal, sx, sy, sz, &area, &el, in, d1nccx, d1nccy,
                      d1nccz, i1eljo[ielem]);
        ...
        Sigma_tau(o, s, sx, sy, sz, normal, area, el, d1nftx, d1nfty, d1nftz,
                  dpeft, dpegfn, dpegfs, dpepe, dpcoh, dpicf[iprop], i2eljp,
                  i2ejjp, i1elpr, i1elty, ielem, i1elbe, tid, index, in, iprop,
                  nelem_t, i2elto_t, i1bd, d1nbp, dbkp, d1ejfcx, d1ejfcy,
                  d1ejfcz, i1eljt, i1elncstep, ncstep, d2ejfjx[ielem],
                  d2ejfjy[ielem], d2ejfjz[ielem]);
 ...
 *
 */
void Yfd(YDE yde, YDN ydn, YDP ydp, YDK ydk, YDI ydi, YDC ydc, YDX ydx, YDJ ydj,
         YDJP ydjp, YDB ydb, int nthreads) {
  ...
  {
    Yfd3TET10ELS(yde->nelemi, ydp->d1peks, ydp->d1pela, ydp->d1pemu, ydn->d2ncc,
                 ydn->d2nci, ydn->d2nft, ydn->d2nvc, yde->i1elpr, yde->i2elto,
                 ydk->d3dsh, nthreads, nblock, ydn->nnopo, nmode);

  } else if ((ydp->i1ptyp[0]) == (YTE3TET4ELS)) {
    Yfd3TET4ELS( // small strain elastic tetrahetra
        yde->nelemi, yde->i1elpr, ydp->d1peks, ydp->d1pela, ydp->d1pemu,
        ydn->d2ncc[0], ydn->d2ncc[1], ydn->d2ncc[2], ydn->d2nci[0],
        ydn->d2nci[1], ydn->d2nci[2], ydn->d2nft, ydn->d2nvc[0], ydn->d2nvc[1],
        ydn->d2nvc[2], yde->i2elto, yde->d1elvol, yde->i1elchprm, yde->d1pem0n,
        ydp->d1pem0, ydp->d1pemf, ydp->d1pedm, yde->d1eldam, yde->d3tst);
  }

  Yfd3TET4JOINT( // joint element for 4-node tetrahedra
      ydj->nelem, ydp->i1ptyp, ydjp->d1peft, ydjp->d1pegfn, ydjp->d1pegfs,
      ydjp->d1pepe, ydn->d2ncc[0], ydn->d2ncc[1], ydn->d2ncc[2], ydn->d2nft[0],
      ydn->d2nft[1], ydn->d2nft[2], ydn->d2nvc[0], ydn->d2nvc[1], ydn->d2nvc[2],
      ydj->i1elpr, ydj->i2elto, ydj->i1eljo, yde->i2eljp, ydj->i2ejjp,
      &(ydi->diedi), yde->i2elfr, yde->i1elbe, yde->i1joint, yde->njoint_intact,
      &(yde->ifrac), ydjp->d1pcoh, ydjp->d1picf, ydj->i1elty, ydj->i1elst,
      &(ydx->nelem_t), ydx->i2elto_t, ydx->i1jtp, ydx->d1nbp, ydx->dbkp,
      ydj->d2ejfc[0], ydj->d2ejfc[1], ydj->d2ejfc[2], ydj->i1defect, nthreads,
      yde->i1eljt, ydj->d1ejmr, yde->i1elncstep, ydc->ncstep, yde->d3tst,
      ydj->d2jst);
  ...
  if (((ydc->ncstep % ydc->icoutf) == 0 && (ydc->ifrag == 1)) ||
    ...
    calFragVol(yde->nelemi, yde->i1elfrg, &(yde->nfrag), yde->d1volfrg,
               ...
               ydn->d2ncc[1], ydn->d2ncc[2], ydn->d2nvc[0], ydn->d2nvc[1],
               ydn->d2nvc[2], yde->nelno, ydc->ichprm, index, indexv,
               ydc->dcrmvx, ydc->dcrmvy, ydc->dcrmvz, ydn->d2nfrm, yde->i1elfrm,
               yde->i1frgrm, ydn->d1ndisp, ydc->dcfrmca, ydn->i1nopr,
               ydb->i1bnvx, ydb->i1bnvy, ydb->i1bnvz, ydn->i1rmgrav,
               yde->d1mart);
    ydc->ifrag = 0;
  }
}
