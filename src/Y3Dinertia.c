  ...
  for (i = 0; i < 3-1; i++) 
   {
    ...
    h = d[i+1];
 //   if (h != 0.0)
    if (fabs(h)>= 1.0e-10) //modification avoiding divided by 0
    {
      for (k = 0; k <= i; k++) {
        d[k] = V[k][i+1]/ h;
      }
  ...
  for (l = 0; l < 3; l++) {
    ...
    if (m > l) {
      ...
      do {
        ...
        if (p < 0) {
          r = -r;
          d[l] = e[l] / (p + r-1.0e-10); //add this avoiding divided by 0
        }
	else d[l] = e[l] / (p + r+1.0e-10); ////add this avoiding divided by 0
        d[l+1] = e[l] * (p + r);
        dl1 = d[l+1];
        h = g - d[l];
        for (i = l+2; i <3; i++) {
        ...
        for (i = m-1; i >= l; i--) {
          ...
          e[i+1] = s * r;
	  r=r+1.0e-10; //plus 1.0e-10 avoiding divided by 0
          s = e[i] / r;
          c = p / r;
          p = c * d[i] - s * g;
          d[i+1] = h + s * (c * g + s * d[i]);
...
void V3DRotAxes(afx,afy,afz,bfx,bfy,bfz,u,v,w)
DBL *afx; DBL *afy; DBL *afz; DBL bfx; DBL bfy;
DBL bfz; DBL u; DBL v; DBL w;
{
    DBL a1,a2,a3,b1,b2,b3;
    a1=COS(u);
    a2=COS(v);
    a3=COS(w);
    b1=SIN(u);
    b2=SIN(v);
    b3=SIN(w);
    (*afx)=bfx*a2*a3+bfy*(a3*b1*b2-a1*b3)+bfz*(a1*b2*a3+b1*b3);
    (*afy)=bfx*a2*b3+bfy*(a1*a3+b1*b2*b3)+bfz*(-a3*b1+a1*b2*b3);
    (*afz)=-bfx*b2+bfy*a2*b1+bfz*a1*a2;
